...
using namespace std;
    ...
    const int simu_step=1001; //1001 //test-part !!can't be 1 or won't run
    ...
    int tp=0;
    double tn = 0.;
    int output_str_step=10, output_ene_step=simu_step-1;//1000; //10 //test-part
    bool clash_flag=false, output_ene_flag=true, output_str_flag=true, comp_rmsd=true;
    int mem[17];
    double trj[8];
    //int index;
            ...
            rmsd = rmsdcall(chainY*2, chainX*2, &TCR_x[0][0],&TCR_y[0][0],&TCR_z[0][0], &TCR_x_0[0][0],&TCR_y_0[0][0],&TCR_z_0[0][0], &MHC_x[0][0],&MHC_y[0][0],&MHC_z[0][0], &MHC_x_0[0][0],&MHC_y_0[0][0],&MHC_z_0[0][0]);
            {
                ...
                for (mc_time_step=0;mc_time_step < simu_step;mc_time_step++) //max simu_step 10000
                {
                    ...
                    else
                    {
                        ...
                        if ((sum_diff2 <= sum_diff) or ((eclash < 1.) and (exp(-(sum_diff2-sum_diff)/simu_temp) > random_d(1.0))))//((sum_diff2+0.1 <= sum_diff) or ( sum_diff2 <= sum_diff+5. and (0.02 > random_d(1.0)))) //((sum_diff2 <= sum_diff) or ((sum_diff2 > sum_diff) and (exp(-(sum_diff2-sum_diff)/simu_temp) > random_d(1.0))))// (sum_diff2-sum_diff)/10. > random_d(1.0))) //
                        {
                            sum_diff=sum_diff2;
